...
namespace ChinaAeroSpaceNearFuturePackage.CASNFPParts.ArmParts
{
    public class RoboticArmController : MonoBehaviour
    {
        ...
        private float convergeThreshold = 0.01f;
        ...
        private int maxUnreachFrame = 100;
        ...
        private void CCDIK(Vector3 targetPos)
        {
            ...
            for (int i = 0; i < ikSpeed; i++)
            {
                ...
                if (distanceToTarget < 0.01f)
                {
                    Debug.Log("机械臂已到达工作位置,break执行" + i + "次");
                    break;
                }
                // 只让effectJoint参与
        ...
        private void CHeckArrived()
        {
            ...
            if (dist > convergeThreshold)
            {
                ...
                {
                    ScreenMessages.PostScreenMessage("取样点" + workTransformTargetPos + "无法到达，机械臂自动回收", 2f, ScreenMessageStyle.UPPER_RIGHT);
                    unreachFrameCount = 0;
        ...
        {
            ScreenMessages.PostScreenMessage("机械臂已到达取样点,开始取样", 2f, ScreenMessageStyle.UPPER_RIGHT);
            hasTargetPos = false;
        ...
        private void HandleIdleState()
        {
            if (!hasTargetPos)
            {
                ScreenMessages.PostScreenMessage("机械臂待命，可以选择取样地点", 3.0f, ScreenMessageStyle.UPPER_RIGHT);
        ...
        {
            ScreenMessages.PostScreenMessage("机械臂正在回收，请稍候！", 3.0f, ScreenMessageStyle.UPPER_RIGHT);
            for (int i = 0; i < joints.Length; i++)
            {
                joints[i].SetAngle(joints[i].initialAngle);
                if (IsRetracted())
                {
                    hasTargetPos = false;
                    controlledArm.ArmCurrentState = ArmState.Idle;
                }
            }
        }
        ...
        {
            targetPosNoramlDir = (hit.point - FlightGlobals.getMainBody().transform.position).normalized;
